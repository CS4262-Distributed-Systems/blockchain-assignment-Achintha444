...
public final class AssetTransfer implements ContractInterface {
  ...
  @Transaction(intent = Transaction.TYPE.SUBMIT)
    public Asset DuplicateAsset(final Context ctx, final String assetID, final String newOwner) {
        ...
        // To check if the asset exists if not show an error
        if (assetJSON == null || assetJSON.isEmpty()) {
            String errorMessage = String.format("Asset %s does not exist", assetID);
            System.out.println(errorMessage);
            throw new ChaincodeException(errorMessage, AssetTransferErrors.ASSET_NOT_FOUND.toString());
        ...
        // To create the duplicated asset
        final String newAssetID = "d-" + assetID;
        final String duplicatedAssetID = "d-" + assetID;

        Asset duplicatedAsset = new Asset(duplicatedAssetID, asset.getColor(), asset.getSize(), newOwner, asset.getAppraisedValue());

        String sortedJson = genson.serialize(newAsset);
        stub.putStringState(duplicatedAssetID, sortedJson);
